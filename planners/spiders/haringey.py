...
class HaringeySpider(scrapy.Spider):
    ...
    def parse(self,response):
        start=0
        for d in dates_str4[0:-1]:
                    
            start += 1
            fd = d
            nd = dates_str4[start]

            self.driver.get('https://publicregister.haringey.gov.uk/pr/s/register-view?c__r=Arcus_BE_Public_Register')
            sleep(3)
            try:
            ...
            try:
                self.driver.find_element('xpath',"//button[@id='combobox-button-18']").click()
        ...
        for i in self.listhref:
            abs_i = f'https://publicregister.haringey.gov.uk{i}&tabset-3892f=3'
