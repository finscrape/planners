...
class WestmorlandandfurnessSpider(scrapy.Spider):
    name = "westmorLandAndFurness"
    ...
    def parse(self, response):
        ...
        for d in dates_str5[0:-1]:
            ...
            self.driver.get('https://planningregister.westmorlandandfurness.gov.uk/Search/Advanced')
            #choose planning applications
            if fd == dates_str5[0]:
                try:
                    self.driver.find_element("xpath","//input[@id='SearchEnforcement']").click()
                except:
                    self.driver.find_element("xpath","//input[@id='SearchEnforcement']").click()
        ...
        for c in self.listhref:
            ...
            abs_i = f'https://planningregister.westmorlandandfurness.gov.uk{c}'
            yield scrapy.Request(url=abs_i,callback=self.final,dont_filter=True)

    def final(self,response):     
        each = {}
        each['planningUrl'] = response.url
        table_hd = response.xpath("//div[@id='Main Details']/descendant::table[1]/descendant::tr")
        for i in table_hd:
        ...
        table_hd = response.xpath("//div[@id='Main Details']/descendant::table[2]/descendant::tr")
        for i in table_hd:
            first = i.xpath(".//td[1]/text()").get()
            if first in ['Summary','Important Dates','Further Information','Condition Details / Information Notes']:
                pass
        ...
        table_hd = response.xpath("//div[@id='Main Details']/descendant::table[3]/descendant::tr")
        for i in table_hd:
            first = i.xpath(".//td[1]/text()").get()
            if first in ['Summary','Important Dates','Further Information','Condition Details / Information Notes']:
                pass
        ...
        empty = []
        table_hd = response.xpath("//div[@id='Main Details']/descendant::table[4]/descendant::tr")
        if table_hd:
            for i in table_hd[1:]:
